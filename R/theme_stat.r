...
theme_stat <- function(base_size = 11, axis.label.pos = "top", axis.lines = "x",
  ...
  theme_var <- ggplot2::theme_minimal(base_family = "Arial") +
    ...
    ggplot2::theme(
      ...
      strip.text = ggplot2::element_text(size = base_size, color = color.axis,
                                         face = "plain", family = "Arial"),
  ...
  theme_var <- theme_var +
    ggplot2::theme(axis.title.x = ggplot2::element_text(color = color.axis,
                                                        vjust = vjust.x,
                                                        hjust = hjust.x),
                   axis.title.y = ggplot2::element_text(color = color.axis,
                                                        vjust = vjust.y,
                                                        hjust = hjust.y))
  ...
  if (ticks != "none") {
    if (ticks == "x") {
      theme_var <- theme_var +
        ggplot2::theme(
          axis.ticks.x = ggplot2::element_line(colour = color.axis,
                                               size = 0.25),
    ...
    if (ticks == "y") {
      theme_var <- theme_var +
        ggplot2::theme(
          axis.ticks.y = ggplot2::element_line(colour = color.axis,
                                               size = 0.25),
          axis.ticks.length = unit(0.1, "cm")
        )
    }
  ...
  if(minor.grid.lines != "none"){
    ...
    if(minor.grid.lines == "x") {
      theme_var <- theme_var +
        ggplot2::theme(
          panel.grid.minor.x = ggplot2::element_line(colour = color.grid,
                                                     size = 0.1)
        )
    }
    ...
    if(minor.grid.lines == "y") {
      theme_var <- theme_var +
        ggplot2::theme(
          panel.grid.minor.y = ggplot2::element_line(colour = color.grid,
                                                     size = 0.1)
        )
    }
  ...
  if(major.grid.lines != "none"){
    ...
    if(major.grid.lines == "x") {
      theme_var <- theme_var +
        ggplot2::theme(
          panel.grid.major.x = ggplot2::element_line(colour = color.grid,
                                                     size = 0.2)
        )
    }
    ...
    if(major.grid.lines == "y") {
      theme_var <- theme_var +
        ggplot2::theme(
          panel.grid.major.y = ggplot2::element_line(colour = color.grid,
                                                     size = 0.2)
        )
    }
