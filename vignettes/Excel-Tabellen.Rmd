...
Um das Paket einem breiteren Anwenderkreis zugänglich zu machen, wurden diverse Aspekte im Design parametrisiert. Textfelder wie Kontaktinformationen können in den Funktionen direkt übersteuert werden. Für längerfristige Anpassungen besteht die Möglichkeit, Nutzerkonfigurationen anzulegen. Die Konfigurationen sind YAML-Dateien, in denen Einträge durch Schlüsselwörter mit zugehörigen Werten hinterlegt sind. 

Bei der Erstinstallation von statR kann mit der Funktion \code{initUserConfigStore} ein Verzeichnis "~/.config/R/statR" erstellt und ein Index-File angelegt werden, in dem Konfigurationen erfasst werden. Das Paket kommt mit einer Standardkonfiguration ("default") mit folgendem Inhalt:

```yaml
`r xfun::file_string(system.file("extdata/config/default.yaml", package = "statR"))`
```

Auf der Grundlage von 'default.yaml' können auch weitere Konfigurationen angelegt und mit der Funktion \code{addUserConfig} registriert werden. Das folgende Beispiel demonstriert einen einfachen Workflow zum Anlegen einer neuen Konfiguration:
