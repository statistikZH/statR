...
einzelnen Arbeitsblatt mit einem Header für ein Logo, Kontaktangaben, Titel,
...
![Screenshot von motor_trend_car_road_tests.xlsx](./quickXLSX.PNG){width=95%}

### aXLSX
Mit der Funktion `aXLSX()` kann ein einzelner Datensatz aus `R` als vorformatierte XLSX-Datei exportiert werden. Die Funktion hat zum Ziel, möglichst __barrierefreie__ Excel-Tabellen zu erstellen, die sich auch für die Publikation auf [zh.ch](zh.ch) eignen (siehe: https://statistikzh.github.io/axguide/grafiken.html#components). Dazu werden Daten und Metadateninformationen in separate Arbeitsblätter abgefüllt. Das erste Arbeitsblatt beinhaltet einen Titel, eine Quellenangabe und die Daten, das zweite Arbeitsblatt Hinweise zu den Daten sowie Kontaktinformationen. Mit dem Argument `author` kann vermerkt werden, wer die Datei erstellt hat. Das Default-Argument 'user' bedeutet, dass die letzten zwei Zeichen de Usernames gemäss dem System verwendet werden. Im Argument können aber auch Vollnamen angegeben werden.
...
![Screenshot des Datenblatts in der Datei motor_trend_car_road_tests.xlsx](./aXLSX_1.PNG){width=95%}
![Screenshot des Metadatenblatts der Datei motor_trend_car_road_tests.xlsx](./aXLSX_2.PNG){width=95%}


### splitXLSX

Mit der Funktion `splitXLSX()` kann ein Datensatz aus R anhand eines Merkmals aufgespalten und über mehrere Arbeitsblätter verteilt weren. Zur Vereinfachung der Navigation wird im ersten Arbeitsblatt ein Inhaltsverzeichnis mit Hyperlinks angelegt. Für jede Ausprägung wird ein eigenes Arbeitsblatt angelegt. 

Die Funktion ist besonders dann geeignet, wenn die Variable eine begrenzte Anzahl von Ausprägungen mit intuitiven Bedeutungen hat (zB. Altersklassen, Jahre oder Regionen). Implizit wird angenommen, dass die Quellangaben und Metadaten für jede der Ausprägungen gelten. Andernfalls sollte die Funktion \code{datasetsXLSX} verwendet werden.


Das folgened Beispiel erstellt eine Excel Arbeitsmappe aus dem \code{mtcars} Dataset. Für jede Ausprägung der Anzahl Zylinder wird ein neues Arbeitsblatt angelegt.
```{r,echo = TRUE, eval = FALSE}
mtcars_title <- "Motor trend car road tests"
mtcars_source <- paste0("Henderson and Velleman (1981). Building multiple ",
                        "regression models interactively. ",
...
![Screenshot: Titelblatt der Datei datasetsXLSX_demo.xlsx](./datasetsXLSX_1.PNG){width=95%}
![Screenshot: Tabelle in 2. Arbeitsblatt der Datei datasetsXLSX_demo.xlsx](./datasetsXLSX_2.PNG){width=95%}
![Screenshot: Grafik in 5. Arbeitsblatt der Datei datasetsXLSX_demo.xlsx](./datasetsXLSX_2.PNG){width=95%}
